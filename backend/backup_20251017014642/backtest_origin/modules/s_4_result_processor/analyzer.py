"""回测结果分析模块 - 负责指标统计、曲线生成与交易记录整理。

该实现从 `backend/app/7_result_pipeline.py` 迁移而来，保持原有行为，以便后续逐步优化。
"""
...
from ..s_2_data_loader import csv_loader as _dl
...
def compute_metrics_and_curves(
    ...
    if eq_filtered:
        ...
        equity_curve = [
            {"date": p["date"], "equity": (p["equity"] / base if base else p["equity"])}
            for p in eq_filtered
        ]
    daily_returns = [
        {"date": dt.strftime("%Y-%m-%d"), "ret": r}
        for dt, r in timereturn.items()
        if dt.strftime("%Y-%m-%d") >= start
    ]
    try:
...
def collect_trade_and_order_details(
    ...
    try:
        raw_records = jq_state.get("records", []) if isinstance(jq_state, dict) else []
        if isinstance(raw_records, list) and raw_records:
            jq_records = []
            last_marker = None
            fallback_marker = jq_state.get("current_dt") if isinstance(jq_state, dict) else None
            if not isinstance(fallback_marker, str) or not fallback_marker.strip():
                fallback_marker = jq_state.get("latest_dt") if isinstance(jq_state, dict) else None
            if not isinstance(fallback_marker, str) or not fallback_marker.strip():
                fallback_marker = jq_state.get("user_start") if isinstance(jq_state, dict) else None
            ...
            for rec in raw_records:
                ...
                if not marker:
                    candidate = last_marker or fallback_marker
                    marker = candidate.strip() if isinstance(candidate, str) and candidate.strip() else None

                entry["dt"] = marker
                if marker:
                    last_marker = marker
                jq_records.append(entry)
    except Exception:
        jq_records = None
    try:
